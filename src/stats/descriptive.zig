...
pub fn mode(comptime T: type, data: ndarray_module.NDArray(T, 1), allocator: Allocator) (ndarray_module.NDArray(T, 1).Error || Allocator.Error)!T {
    ...
    // Use AutoHashMap to count frequencies
...
test "mode: single frequency f64" {
    const allocator = testing.allocator;
    const data_slice = [_]f64{ 1.0, 2.0, 2.0, 3.0 };
    var data = try ndarray_module.NDArray(f64, 1).fromSlice(allocator, &[_]usize{4}, &data_slice, .row_major);
    defer data.deinit();
    const result = try mode(f64, data, allocator);
    try testing.expectApproxEqAbs(2.0, result, 1e-10);
}
...
test "mode: all same values f64" {
    const allocator = testing.allocator;
    const data_slice = [_]f64{ 7.0, 7.0, 7.0, 7.0 };
    var data = try ndarray_module.NDArray(f64, 1).fromSlice(allocator, &[_]usize{4}, &data_slice, .row_major);
    defer data.deinit();
    const result = try mode(f64, data, allocator);
...
test "mode: multiple modes returns first f64" {
    const allocator = testing.allocator;
    // 1.0 appears 2 times, 2.0 appears 2 times — should return first encountered in counting
    const data_slice = [_]f64{ 1.0, 2.0, 1.0, 2.0 };
    var data = try ndarray_module.NDArray(f64, 1).fromSlice(allocator, &[_]usize{4}, &data_slice, .row_major);
    defer data.deinit();
    const result = try mode(f64, data, allocator);
    // Result should be either 1.0 or 2.0 (first encountered with max frequency)
    try testing.expect(result == 1.0 or result == 2.0);
}

test "mode: negative values f64" {
    const allocator = testing.allocator;
    const data_slice = [_]f64{ -5.0, -5.0, 0.0, 5.0 };
    var data = try ndarray_module.NDArray(f64, 1).fromSlice(allocator, &[_]usize{4}, &data_slice, .row_major);
    defer data.deinit();
    const result = try mode(f64, data, allocator);
    try testing.expectApproxEqAbs(-5.0, result, 1e-10);
...
test "mode: float with duplicates f32" {
    const allocator = testing.allocator;
    const data_slice = [_]f32{ 1.5, 1.5, 2.5, 3.5 };
    var data = try ndarray_module.NDArray(f32, 1).fromSlice(allocator, &[_]usize{4}, &data_slice, .row_major);
    defer data.deinit();
    const result = try mode(f32, data, allocator);
    try testing.expectApproxEqAbs(1.5, result, 1e-5);
}
